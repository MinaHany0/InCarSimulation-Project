...
typedef enum{
	KS_125 = 1 , KS_250 = 3 , KS_500 = 5, MS_1 = 7
...
/* Type for assignment of channels to a channel group */																		//STILL NOT DONE HOW MANY VALUES ? need uint 8 or 16??
/*typedef struct{
...
typedef struct{
	boolean 							Adc_Module_ID			;	 	// ADC_MODULE_0 / ADC_MODULE_1
	uint8								Adc_Module_Sequencer	;	 	// ADC_SEQUENCER_0/ ../ ADC_SEQUENCER_3
	Adc_GroupType						Adc_Module_Group_ID		;	 	// ADC_GROUP_0 / .. / ADC_GROUP_3
	uint8								Adc_Group_Channel_No	;	 	// 1 / .. / 8
	Adc_ChannelType*					Adc_Channels			;	 	// pointer to array identifying channels used in group
	Adc_ValueGroupType*					Adc_Group_Buffer		;	 	// pointer to array of buffers for channels
	Adc_TriggerSourceType				Adc_Group_Trigg_Src		;	 	// ADC_TRIGG_SRC_SW / ADC_TRIGG_SRC_HW
	Adc_ResolutionType					Adc_Resolution			;	 	// (12 DEFAULT) 1 / .. / 12 bit resolution
	Adc_PrescaleType					Adc_HWOverSampling		;	 	// ADC_NoHardwareOverSampling / .. / ADC_64xHardwareOversampling
	Adc_SamplingTimeType                Adc_Sampling_Time       ;       // 125 KSPS, 250 KSPS, 500 KSPS, 1 MSPS
	Adc_GroupConvModeType				Adc_GroupConv			;	 	// ADC_CONV_MODE_ONESHOT / ADC_CONV_MODE_CONTINUOUS
	Adc_GroupPriorityType				Adc_GroupPriority		;	 	// ADC_GroupPriority_0/../ADC_GroupPriority_3 (Highest priority is 0)
	Adc_StreamBufferModeType			Adc_StreamBufferMode	;	 	// ADC_STREAM_BUFFER_LINEAR / ADC_STREAM_BUFFER_CIRCULAR
	Adc_GroupAccessModeType				Adc_GroupAccessMode		;	 	// ADC_ACCESS_MODE_SINGLE / ADC_ACCESS_MODE_STREAMING
	Adc_HwTriggerSignalType				Adc_HwTrigger			;	 	// ADC_HW_TRIG_RISING_EDGE / ADC_HW_TRIG_FALLING_EDGE / ADC_HW_TRIG_BOTH_EDGES
	Adc_PriorityImplementationType		Adc_PrioImplementation	;		// ADC_PRIORITY_NONE / ADC_PRIORITY_HW / ADC_PRIORITY_HW_SW
	Adc_GroupReplacementType			Adc_GroupReplacement	;		// ADC_GROUP_REPL_ABORT_RESTART
	Adc_ChannelRangeSelectType			Adc_ChannelRangeSelect	;		// ADC_RANGE_ALWAYS / ...
	Adc_ResultAlignmentType				Adc_ResultAlignment 	;		// ADC_ALIGN_LEFT / ADC_ALIGN_RIGHT
	Adc_PowerStateType					Adc_PowerState			;		// ADC_FULL_POWER / ADC_POWER_1 / ADC_POWER_2
}Adc_ConfigGroup;	

typedef struct{
	Adc_ConfigGroup Groups[ADC_CONFIGURED_GROUP];
}Adc_ConfigType;
